...
export default function AdminContent({ reports, reviews }: AdminContentProps) {
  ...
  return (
    <div className='flex flex-col gap-6 h-full'>
      ...
      <div
        className={
          gridView
            ? "grid grid-rows-2 grid-cols-3 lg:grid-rows-1 lg:grid-cols-1 gap-6 w-full items-center"
            : "flex flex-col rounded-md justify-center gap-2"
        }
      >
        {reports ? (
          reports
            ...
            .map((r: Report, index: number) => (
              <ReportCard
                ...
                report={r}
                review={
                  reviews.find((review) => review.reviewId === r.reviewId)!
                }
                gridView={gridView}
              />
            ))
          ...
          <span
            className={`text-sm ${
              gridView ? "text-left" : "text-center"
            } text-gray-800`}
          >
            No reports found
          </span>
